...
poor_inf_col      = '#36F3CB';
% check if Matlab version is pre 2018, if so, convert colours to RGB
v = version; year = str2num(v(end-4:end-2));
if year < 19
    card_1_col          = sscanf(card_1_col(2:end),'%2x%2x%2x',[1 3])/255;
    card_2_col          = sscanf(card_2_col(2:end),'%2x%2x%2x',[1 3])/255;
    trust_col           = sscanf(trust_col(2:end),'%2x%2x%2x',[1 3])/255;
    not_trust_col       = sscanf(not_trust_col(2:end),'%2x%2x%2x',[1 3])/255;
    corr_feedback_col   = sscanf(corr_feedback_col(2:end),'%2x%2x%2x',[1 3])/255;
    false_feedback_col  = sscanf(false_feedback_col(2:end),'%2x%2x%2x',[1 3])/255;
    posterior_col       = sscanf(posterior_col(2:end),'%2x%2x%2x',[1 3])/255;
    poor_inf_col        = sscanf(poor_inf_col(2:end),'%2x%2x%2x',[1 3])/255;
end    
...
try
    ...
    text(ax,0,0,'{Decision \newlinetrajectory}','HorizontalAlignment','right');
end
try
    text(ax,0,max_pos+sep + 0.5*(max_total-max_pos-sep),card_advice_message,'HorizontalAlignment','right');
catch
    text(ax,0,max_pos+sep + 0.5*(max_total-max_pos-sep),'{Advised \newlinecard}','HorizontalAlignment','right');
end
try
    text(ax,0,max_pos/2,card_choice_message,'HorizontalAlignment','right');
catch
    text(ax,0,max_pos/2,'{Chosen \newlinecard}','HorizontalAlignment','right');
end
try
    text(ax,0,-max_pos/2,feedback_message,'HorizontalAlignment','right');
catch
    text(ax,0,-max_pos/2,'{Feedback \newlinereceived}','HorizontalAlignment','right');
end
try
    text(ax,0,-max_total+0.5*(max_total-max_pos-sep),advisor_message,'HorizontalAlignment','right');
catch
    text(ax,0,-max_total+0.5*(max_total-max_pos-sep),'{Advisor \newlinetrustworthiness}','HorizontalAlignment','right');
end
